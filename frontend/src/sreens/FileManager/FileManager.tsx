...
const FileManager = () => {
  ...
  // const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>, parentId: string) => {
  //   const selectedFiles = e.target.files;
  //   if (!selectedFiles || selectedFiles.length === 0) return;

  //   // Создаем массив новых файлов для добавления
  //   const newFiles: FileNode[] = Array.from(selectedFiles).map(file => ({
  ...
  //     // Особый случай для корневой папки
  //     if (parentId === 'root') {
  //       const root = prevFiles.find(n => n.id === 'root');
  //       if (root) {
  //         return prevFiles.map(node => 
  //           node.id === 'root' 
  //             ? { ...node, children: [...(node.children || []), ...newFiles] }
  //             : node
  //         );
  //       } else {
  //         return [{
